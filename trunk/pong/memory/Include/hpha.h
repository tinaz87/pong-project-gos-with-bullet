...
class allocator
{
	...
	public:
		...
		debug_record* replace_begin(void* ptr);
		debug_info replace_end(debug_record* dr, void* ptr, size_t size, debug_source source);
	...
	debug_record* debug_replace_begin(void* ptr);
	void debug_replace_end(debug_record* dr, void* ptr, size_t size, debug_source source);
	...
	debug_record* debug_replace_begin(void*) {return NULL;}
	void debug_replace_end(debug_record*, void*, size_t, debug_source) {}
	...
	allocator();
	static allocator* ms_singleton;
public:
	static allocator* GetSingletonPtr();
	...
	void* realloc(void* ptr, size_t size) {
		...
		if (ptr_in_bucket(ptr)) {
			size = clamp_small_allocation(size);
			if (is_small_allocation(size)) {
				debug_record* dr = debug_replace_begin(ptr);
				void* newPtr = bucket_realloc(ptr, size + MEMORY_GUARD_SIZE);
				debug_replace_end(dr, newPtr, size, DEBUG_SOURCE_BUCKETS);
				return newPtr;
			}
			debug_record* dr = debug_replace_begin(ptr);
			void* newPtr = tree_alloc(size + MEMORY_GUARD_SIZE);
			debug_replace_end(dr, newPtr, size, DEBUG_SOURCE_TREE);
			if (!newPtr)
				return NULL;
