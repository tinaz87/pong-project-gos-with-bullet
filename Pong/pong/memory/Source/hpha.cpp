...
allocator* allocator::ms_singleton= 0;

void allocator::CreateInstance()
{
	GetSingletonPtr();
}
...
allocator* allocator::GetSingletonPtr()
{ 
	if(ms_singleton == NULL)
		ms_singleton= new allocator();
	return ms_singleton; 
}
...
allocator::debug_record* allocator::debug_record_map::replace_begin(void* ptr) {
	iterator it = this->find(ptr);
	// if this asserts most likely the pointer was already deleted
	assert(it != this->end());
	debug_record* dr = it;
...
allocator::debug_info allocator::debug_record_map::replace_end(debug_record* dr, void* ptr, size_t size, debug_source source) {
	size_t origSize = dr->size();
	debug_source origSource = dr->source();
	if (ptr)
...
allocator::debug_record* allocator::debug_replace_begin(void* ptr) {
	#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
	#endif
	return mDebugMap.replace_begin(ptr);
}

void allocator::debug_replace_end(debug_record* dr, void* ptr, size_t size, debug_source source) {
	#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
	#endif
	assert(size <= this->size(ptr));
	debug_info info = mDebugMap.replace_end(dr, ptr, size, source);
	if (!ptr)
		return;
	if (info.source == DEBUG_SOURCE_BUCKETS) {
		mTotalRequestedSizeBuckets -= info.size + MEMORY_GUARD_SIZE;
...
void allocator::debug_check(void* ptr) const {
	#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
	#endif
	mDebugMap.check(ptr);
